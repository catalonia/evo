...
public class TokenGenerator {
	...
	public Response getOnlineToken(@PathParam("param") String user_name) 
	{
		...
		if(status == null || status.isEmpty())
		{
			try 
			{
				...
				long alteredTime = currentTimeMillis + 60*60*1000;
				java.util.Date date= new java.util.Date(alteredTime);
				Timestamp time=new Timestamp(date.getTime());
				connection.insertTokens(_token,user_name,time);
	...
	public Response getOfflineToken(@PathParam("param") String user_name) 
	{
		ResteasyClient client = null;
		String status = connection.checkTokenExist(user_name);
		if(status == null || status.isEmpty())
